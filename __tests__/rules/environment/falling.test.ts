...
describe('Falling Damage', () => {
  beforeEach(() => {
    ...
    vi.spyOn(diceLib, 'rollFromNotation').mockImplementation((notation) => {
      if (notation === '1d6') return [6];
      if (notation === '2d6') return [3, 4];
      if (notation === '3d6') return [3, 3, 3];
      if (notation === '1d20') return [5]; // Make sure this is low enough to cause injury
      return [5]; // Default
    });
    
    vi.spyOn(diceLib, 'sumDice').mockImplementation((results) => {
