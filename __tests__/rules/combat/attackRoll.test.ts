...
describe('Combat Attack Roll Mechanics', () => {
  ...
  describe('attackRoll', () => {
    beforeEach(() => {
      ...
      vi.mocked(diceLib.rollFromNotation).mockReturnValue([5]); // Damage roll
      vi.mocked(diceLib.sumDice).mockReturnValue(5); // Total damage
    });
    ...
    it('should return correct result for a hit', () => {
      ...
      expect(result.message).toContain('6 damage');
    });
