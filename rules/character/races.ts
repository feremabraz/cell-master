 ...
 */
export const getRaceLevelLimits = (race: CharacterRace, characterClass: CharacterClass, strength: number, intelligence: number, dexterity: number): number | null => {
  ...
  const levelLimits: Partial<Record<CharacterRace, Partial<Record<CharacterClass, number | null | ((attrs: AbilityScores) => number)>>>> = {
    ...
    'Elf': {
      ...
      'Illusionist': null,
      'Magic-User': (attrs: AbilityScores) => attrs.intelligence >= 18 ? 11 : attrs.intelligence >= 17 ? 10 : 9,
      'Paladin': null,
      'Ranger': null,
      'Thief': null // Unlimited
    },
...
export const getRacialAbilities = (race: CharacterRace): RacialAbility[] => {
  const abilities: Record<CharacterRace, RacialAbility[]> = {
    'Human': [
      {
        ...
        effect: () => { /* No specific effect needed */ }
      }
    ],
    'Dwarf': [
      {
        ...
        effect: () => { /* Apply magic resistance based on Con */ }
      },
      {
        name: 'Poison Resistance',
        description: 'Bonus against poison based on constitution: +1 per 3.5 points of Con',
        effect: () => { /* Apply poison resistance based on Con */ }
      },
      {
        name: 'Combat Bonus vs. Goblinoids',
        description: '+1 to hit against goblins, half-orcs, hobgoblins, and orcs',
        effect: () => { /* Apply combat bonus against specified enemies */ }
      },
      {
        name: 'Defense Bonus vs. Giants',
        description: '-4 penalty to attack rolls made against the dwarf by giants, ogres, ogre mages, titans and trolls',
        effect: () => { /* Apply defense bonus against specified enemies */ }
      },
      {
        name: 'Stonework Detection',
        description: 'Ability to detect certain features of stonework',
        effect: () => { /* Apply stonework detection abilities */ }
      }
    ],
    'Elf': [
      {
    ...
    'Gnome': [
      ...
      {
        name: 'Stonework Detection',
        description: 'Ability to detect certain features of stonework and determine depth/direction underground',
        effect: () => { /* Apply stonework detection abilities */ }
      },
      {
        name: 'Speak with Burrowing Animals',
        description: 'Ability to communicate with normal burrowing animals',
        effect: () => { /* Apply animal communication ability */ }
      }
    ],
    'Half-Elf': [
      {
    ...
    'Halfling': [
      ...
      },
      {
        ...
        effect: () => { /* Apply missile weapon bonus */ }
      },
      {
        name: 'Surprise Bonus',
        description: 'Bonus to surprise enemies when alone or with elves/halflings',
    ...
    'Half-Orc': [
      {
        ...
        effect: () => { /* Apply strength and constitution adjustments */ }
      }
    ]
  };
