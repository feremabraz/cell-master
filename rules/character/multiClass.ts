...
export const MULTICLASS_RESTRICTIONS: Record<CharacterRace, string> = {
  ...
  'Half-Elf': 'The less restrictive of any class requirements apply, except that thieving abilities can only be used while wearing armor permitted to thieves.',
  Halfling: 'Use of thieving abilities is only possible when wearing armor permitted to thieves.',
  'Half-Orc': 'For armor, the more restrictive of any two class requirements apply. For weapons, the less restrictive requirements apply.',
  Human: 'Humans cannot multi-class, but may dual-class after reaching at least 2nd level in their original class.'
};
...
export function canDualClass(
  ...
  if (!meetsClassRequirements(abilityScores, originalClass) || 
