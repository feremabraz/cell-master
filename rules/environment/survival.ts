...
export const resolveSurvivalNeeds = (params: SurvivalNeedParams): SurvivalNeedResult => {
  ...
  if (newStatus.daysSinceLastFood > 0) {
    ...
    if (newStatus.daysSinceLastFood >= foodThreshold) {
      ...
      // Add stat penalties
      const statPenaltyValue = Math.min(6, Math.floor((newStatus.daysSinceLastFood - foodThreshold) / 2) + 1);
  ...
  if (newStatus.daysSinceLastWater > 0) {
    ...
    if (newStatus.daysSinceLastWater === 1) {
      ...
      const statPenaltyValue = Math.min(8, Math.floor((newStatus.daysSinceLastWater - waterThreshold) * 2) + 2);
      ...
      statPenalties.constitution = Math.max(statPenalties.constitution || 0, Math.floor(statPenaltyValue / 1.5));
