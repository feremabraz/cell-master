...
export function canMemorizeSpell(character: Character, spell: Spell): boolean {
  ...
  if (spell.class === 'Magic-User' || spell.class === 'Illusionist') {
    const hasSpellInBook = character.spellbook.some(
      bookSpell => bookSpell.name === spell.name
    );
...
export function getTotalSpellSlots(character: Character): SpellSlots {
  ...
  if (!character.class || !['Magic-User', 'Cleric', 'Druid', 'Illusionist'].includes(character.class)) {
    return {};
  }
...
export function memorizeSpell(
  ...
  if (options.enforceSlotLimits && memorizedCounts[spell.level] >= (availableSlots[spell.level] || 0)) {
    return {
      success: false,
