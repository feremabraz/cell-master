...
export const advanceTimeAtom = atom(
  ...
  (
    ...
    set,
    {
      amount,
      unit,
    }: { amount: number; unit: 'Round' | 'Turn' | 'Hour' | 'Day' | 'Week' | 'Month' | 'Year' }
  ) => {
...
// Toggle combat state
export const toggleCombatAtom = atom(null, (get, set) => {
  const inCombat = get(inCombatAtom);
  set(inCombatAtom, !inCombat);
});

// Add a timed effect
export const addTimedEffectAtom = atom(null, (get, set, effect: TimedEffect) => {
  const effects = get(activeEffectsAtom);
  set(activeEffectsAtom, [...effects, effect]);
});

// Remove a timed effect by ID
export const removeTimedEffectAtom = atom(null, (get, set, effectId: string) => {
  const effects = get(activeEffectsAtom);
  set(
    activeEffectsAtom,
...
export const resetToNewDayAtom = atom(null, (get, set) => {
  const currentTime = get(gameTimeAtom);
  set(gameTimeAtom, {
    ...currentTime,
    rounds: 0,
